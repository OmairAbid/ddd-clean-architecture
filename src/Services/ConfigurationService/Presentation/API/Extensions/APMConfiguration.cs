...
{
    public static IServiceCollection AddAPM(this IServiceCollection services, IConfiguration configuration)
    {
        var tracingExporter = configuration.GetValue<string>("UseTracingExporter").ToLowerInvariant();

        var serviceName = configuration.GetValue<string>("Logging:OpenTelemetry:ServiceName").ToString();
